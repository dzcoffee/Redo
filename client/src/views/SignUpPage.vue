<template>
    <v-container class="signup-page d-flex flex-column align-center justify-center h-100">
        <v-card class="py-5 px-8">
            ...
            <InfoInput v-bind:valid="isDuplicated" title="비밀번호 확인" placeholder="다시 입력해주세요." validText="10자 이내로 비밀번호와 일치하게 입력하세요." 
            @update-input="onChangeCheckPassword"></InfoInput>
...
const isValidInputLength = computed(() => newUser.value.accountID.length <= LIMIT_LENGTH 
&& newUser.value.nickname.length <= LIMIT_LENGTH && newUser.value.password.length <= LIMIT_LENGTH);
